...
- 缺失字段、全零 UUID 和无效 MAC 不能作为新配置保存。不完整的旧配置仍可查看和导出，但重新导入时需要先修正这些值。
...
仓库中的 `hipreport.conf.example` 将设备名称、GUID、MAC、账号和客户端 IP 留空。真实值只放在私有配置中；测试只检查参考文件为空、生成结果有效，不保存维护者的实际标识作为校验常量或测试样本。
